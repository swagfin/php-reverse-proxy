...
//#FOR RANDOM (Randomized Load Balancer)
$proxyTo = $servers[array_rand($servers)];
//exit("Proxxied to: $proxyTo");

//#FOR PARTITION BASED (Using IP Address as Partition Key)
//$partitionKey = $_SERVER['HTTP_HOST'];
//$proxyTo = getPartitionRoute($partitionKey, $servers);
...
$uri_rel = "subdir/index.php"; # URI to this file relative to public_html
...
//New Feature
function getPartitionRoute($key, $routes)
{
	$string = md5($key);
	$bytes = array();
	...
	$maxPartitions =count($routes);
	$firstByte = $bytes[0];
	$unMappedPartitionNr = $firstByte;
	$unMappedPartitionNr = abs($unMappedPartitionNr);
	$partitionIndex = $unMappedPartitionNr % $maxPartitions;
//Response
	return $routes[$partitionIndex];
}
